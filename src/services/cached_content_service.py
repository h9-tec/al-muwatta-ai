...
class CachedContentService:
    ...
    async def get_surah_from_cache(
        self, surah_number: int, edition: str = "quran-uthmani"
    ) -> dict[str, Any] | None:
        """
    ...
    async def get_ayah_from_cache(
        self, ayah_reference: str, edition: str = "quran-uthmani"
    ) -> dict[str, Any] | None:
        """
    ...
    async def search_quran_in_cache(
        self, query: str, edition: str = "quran-uthmani", limit: int = 5
    ) -> list[dict[str, Any]]:
        """
    ...
    async def get_hadith_from_cache(
        self, collection: str, hadith_number: int
    ) -> dict[str, Any] | None:
        """
    ...
    async def search_hadith_in_cache(
        self, query: str, collections: list[str] = None, limit: int = 10
    ) -> list[dict[str, Any]]:
        """
    ...
    async def get_maliki_hadiths_from_cache(self, limit: int = 100) -> list[dict[str, Any]]:
        """
    ...
    async def get_context_for_question(
        ...
        if include_quran:
            ...
            arabic_results = await self.search_quran_in_cache(
                question, edition="quran-uthmani", limit=max_results
            )
        ...
        if include_hadith:
            ...
            hadith_results = await self.search_hadith_in_cache(
                question, collections=["bukhari", "muslim", "malik"], limit=max_results
            )
    ...
    async def format_context_for_llm(
        self, context: dict[str, Any], language: str = "arabic"
    ) -> str:
        """
...
# Singleton instance
_cached_content_service: CachedContentService | None = None
